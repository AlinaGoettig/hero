package de.htwg.se._2.controller
...
import de.htwg.se._2.model.{Board, Cell, Player}
import de.htwg.se._2.utill.Observable
...
class Controller() extends Observable{
    ...
    def creatureliststart(player: Vector[Player]): Vector[(Vector[Int], Cell)] = Vector(
        Vector(0, 0) -> Cell("HA.", "2-3", 10, 2, style = false, 28, player(0)), //5
        Vector(groeße(0), 0) -> Cell(".FA", "1-2", 2, 2, style = false, 44, player(1)), //7
        Vector(0, 1) -> Cell("MA.", "4-6", 10, 2, style = true, 28, player(0)), //5
        Vector(groeße(0), 1) -> Cell("MAG", "2-4", 13, 2, style = true, 20, player(1)), //6
        Vector(0, 2) -> Cell("RO.", "3-6", 10, 3, style = false, 18, player(0)), //6
        Vector(groeße(0), 2) -> Cell(".CE", "2-7", 25, 3, style = false, 10, player(1)), //8
        /*
        Vector(0, 5) -> Cell("AN.", "50", 250, 12, style = false, 2, player(0)), //18
        Vector(groeße(0), 5) -> Cell(".DE", "30-40", 200, 11, style = false, 2, player(1)), //17
        Vector(0, 8) -> Cell("CH.", "20-25", 100, 6, style = false, 4, player(0)), //9
        Vector(groeße(0), 8) -> Cell(".EF", "16-24", 90, 9, style = false, 4, player(1)), //13
        Vector(0, 9) -> Cell("ZE.", "10-12", 24, 5, style = true, 6, player(0)), //7
        Vector(groeße(0), 9) -> Cell(".PI", "13-17", 45, 5, style = false, 6, player(1)), //7
        Vector(0, 10) -> Cell("CR.", "7-10", 35, 4, style = false, 8, player(0)), //6
        Vector(groeße(0), 10) -> Cell(".HO", "7-9", 40, 4, style = false, 8, player(1))*/) //6
    ...
    def createCreatureList(): Vector[Cell] = {
        val field = board.field
        Vector(field(0)(0),field(0)(groeße(0)),field(1)(0),field(1)(groeße(0)),field(2)(0),field(2)(groeße(0))/*,field(5)(0),
    ...
    def printSidesStart(): String = {
        ...
        val middle = player1 + " " * (50 - player1.length - player2.length) + player2
            ...
            for (j <- 0 to groeße(0)) {
                if (!field(i)(j).name.equals("   ") && !field(i)(j).name.equals(" _ ") && !field(i)(j).name.equals("XXX") && !active(i, j)) {
                    if (((i - 1 >= 0 && j - 1 >= 0) && field(i - 1)(j - 1).name.equals(" _ ")) ||
                        ...
                        ((i - 1 >= 0 && j + 1 <= groeße(0)) && field(i - 1)(j + 1).name.equals(" _ ")) ||
                        ((i - 1 >= 0 && j >= 0) && field(i - 1)(j).name.equals(" _ ")) ||
                        ((i + 1 <= groeße(1) && j >= 0) && field(i + 1)(j).name.equals(" _ ")) ||
                        ((i + 1 <= groeße(1) && j - 1 >= 0) && field(i + 1)(j - 1).name.equals(" _ ")) ||
                        ((i >= 0 && j + 1 < groeße(0)) && field(i)(j + 1).name.equals(" _ ")) ||
                        ((i + 1 <= groeße(1) && j + 1 <= groeße(0)) && field(i + 1)(j + 1).name.equals(" _ "))) {
